	...
	execute: async (interaction: Interaction) => {
		...
		const list = q.songs
			.map(({ title, source }, index) => {
				const icon = AUDIO_MESSAGES.QUEUE.SOURCE_ICON[source];
				return index === 0
					? AUDIO_MESSAGES.QUEUE.CURRENT_SONG(title, icon)
					: AUDIO_MESSAGES.QUEUE.SONG_ITEM(index, title, icon);
			})
			.join("\n");
		...
		logger.info(
			`[${guildId}] [${user}] Viewed queue (${q.songs.length} songs)`,
		);
		await i.reply({
