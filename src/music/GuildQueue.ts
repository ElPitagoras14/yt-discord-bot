...
import type {
	AudioResourceHandle,
	AudioStrategyResolver,
} from "./interfaces.js";
import { GuildQueueState, type Song } from "./types.js";
...
export class GuildQueue {
	...
	constructor(
		...
		resolveStrategy: AudioStrategyResolver,
		onDestroy: () => void,
	) {
	...
	enqueue(song: Song): void {
		...
		logger.info(
			`[${this.guildId}] Enqueued: "${song.title}" (requested by ${song.requestedBy ?? "unknown"})`,
		);
	}
	...
	clear(): void {
		...
		logger.info(
			`[${this.guildId}] Queue cleared (${removed.length} songs removed)`,
		);
	}
	...
	private async playNext(): Promise<void> {
		...
		try {
			this.currentHandle?.cleanup();
			const handle = this.resolveStrategy(song.source).createResource(song);
	...
	private setupPlayerHandlers(): void {
		...
		this.audioPlayer.player.on("error", (error: AudioPlayerError) => {
			...
			const metadata = error.resource?.metadata as
				| { title?: string }
				| undefined;
			if (metadata?.title) {
	...
	private setupConnectionHandlers(): void {
		this.connection.on(VoiceConnectionStatus.Disconnected, async () => {
			try {
				...
				logger.warn(
					`[${this.guildId}] Voice connection lost, destroying queue`,
				);
				this.destroy();
			}
		});
	...
	private startIdleTimer(): void {
		...
		if (!Number.isInteger(duration) || duration <= 0) {
			logger.error(
				`[${this.guildId}] Invalid idle timeout duration: ${duration}`,
			);
			return;
		}
		this.idleTimer = setTimeout(() => {
			...
			this.textChannel
				...
				.catch((err) =>
					logger.error(`[${this.guildId}] Failed to send idle message: ${err}`),
				);
			this.destroy();
		}, duration);
	}
